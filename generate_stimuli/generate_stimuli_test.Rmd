...
split_stimuli <- stimuli %>%
  mutate(value = sample(c("stimulus_1", "stimulus_2", "stimulus_3", 
                          "stimulus_4", "stimulus_5"), nrow(.), replace = TRUE),
         stimulus = paste("'", stimulus, "',", sep = "")) %>%
  distinct(stimulus, .keep_all = TRUE) %>%
  
  # get only 3-letter stimuli
  filter(nchar(stimulus) == 6)
...
stimuli_5 <- split_stimuli %>% 
  filter(value == "stimulus_5") %>%
  select(stimulus) %>%
  t() %>%
  toupper() 
...
stimuli_5 %>% write.table("stimuli_5.txt", quote = FALSE, sep = " ")
